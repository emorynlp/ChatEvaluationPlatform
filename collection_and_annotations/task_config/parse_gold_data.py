...
if __name__ == '__main__':
    ...
    for file in FILES:
        ...
        for idx, sheet in enumerate(content):
            ...
            dialogue_id = f'{task}_gold_{sheet.title}'
            gold_task = {"dialogue_id": dialogue_id,
            ...
            for i, (s, e, t) in enumerate(zip(selections, explanations, turns)):
                ...
                turn_data = [agent, t, s, e] if agent == 'system' else [agent, t]
                gold_task["turns"].append(turn_data)
            gold_tasks.append({"data": [gold_task]})
        with open(f'gold_data/{FINAL_NAME_MAPPING.get(task, task)}.jsonl', 'w') as f:
            for t in gold_tasks:
